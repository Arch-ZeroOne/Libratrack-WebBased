...
export const registerAccount = async (account) => {
  ...
  const rows = await query(
    "INSERT INTO accounts (username,email,password,phone,school_id,verified_at,created_at) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *",
...
export const handleLogin = async (account) => {
  ...
  const rows = await query(
    "SELECT * FROM accounts WHERE email = $1 OR username = $1 AND password = $2",
    [email, password]
  );
  return rows;
};
